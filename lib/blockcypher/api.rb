module BlockCypher
  ...
  class Api
    ...
    def transaction_sign_and_send(new_tx, private_keys)
      ...
      new_tx['pubkeys'], new_tx['signatures'] = signer(new_tx, private_keys)
    ...
    def signer(tx, private_keys)
      ...
      pubkeys = []
      signatures = []
      tx['tx']['inputs'].each_index do |i|
        address = tx['tx']['inputs'][i]['addresses'].first
        pubkeys << pubkey_from_priv(private_keys[address])
    ...
    def event_webhook_subscribe(url, event,
                                confirmations: nil, hash: nil, address: nil,
                                script: nil, wallet_name: nil, token: nil)
      payload = {
        url: url,
        event: event
      }
      ...
      payload[:wallet_name] = wallet_name if wallet_name
